...
- **Bash:**
  ...
  - `biome check .` (verify merged code passes lint)
...
5. **Verify the merge:**
   ...
   biome check .     # Lint must be clean after merge
   ```
6. **Report the result:**
   ```bash
...
- **Always verify after merge.** Run `bun test` and `biome check` after every merge. A merge that breaks tests is not complete.
